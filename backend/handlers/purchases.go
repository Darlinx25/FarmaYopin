...
func GetPurchases(c *gin.Context) {
	...
	rows, err := db.DB.Query(
		"SELECT id, total, payment_method, card_last4, created_at FROM sales WHERE user_id = ? ORDER BY created_at DESC",
		userID,
	)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&pu.ID, &pu.Total, &pu.PaymentMethod, &pu.CardLast4, &pu.CreatedAt); err != nil {
	...
	for i := range purchases {
		itemRows, err := db.DB.Query(`
			SELECT si.product_id, p.name, si.quantity, si.unit_price, COALESCE(p.image_url, '')
			FROM sale_items si
		...
		for itemRows.Next() {
			...
			if err := itemRows.Scan(&it.ProductID, &it.Name, &it.Quantity, &it.UnitPrice, &it.ImageURL); err != nil {
